...
* Purpose: This is the "main" program for our sockets tester.  It parses command line
* parameters and starts up the tester class.
**
...
class test_bcast_spocket : public virtual unit_base, public virtual application_shell
{
public:
  test_bcast_spocket() {}
  DEFINE_CLASS_NAME("test_bcast_spocket");
  virtual int execute();
  bool parse_address(const astring &input, ip_address_holder &ip_address);
};

//hmmm: extract this to function in sockets.
bool test_bcast_spocket::parse_address(const astring &input, ip_address_holder &ip_address)
{
...
int test_bcast_spocket::execute()
{
  FUNCDEF("execute");
